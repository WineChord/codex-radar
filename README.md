...
### v0.1.67：网络波动时稳定保留本机额度

- 本机额度遇到可恢复的网络或连接中断时，会在当次刷新中自动重试一次，减少瞬时代理波动造成的错误闪现。
- 如果重试后仍未恢复，菜单会明确说明下方保留的是最近一次有效数据，并继续自动刷新；不再直接展示内部请求地址和底层英文错误。
- 未登录等需要用户处理的状态不会重试或被隐藏；取消中的旧刷新也不会覆盖较新的结果。
...
- 独立与受管两条本机连接都改用可失败的安全写入；后续刷新仍可重新建立会话，现有重置卡授权、防重复与默认关闭边界保持不变。
- 增加输入通道提前关闭的确定性回归覆盖，确保故障只影响当次连接，不破坏应用的持续运行。

<details>
<summary><strong>历史版本</strong> — 展开查看更早的产品里程碑</summary>

- **v0.1.64**：升级或重启后优先复用已登录的本机受管会话，失败时安全回退到独立 app-server，并提供清楚的登录恢复提示。
