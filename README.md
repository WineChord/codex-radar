...
点击底部 `布局` 可调整顶层模块：拖动手柄或使用上移、下移按钮改变顺序，并为每项选择是否默认展开。模块内还有可折叠子项时，会在所属模块下方缩进显示，例如 `Codex 额度` 下的 `额度历史` 和 `Codex IQ` 下的 `全部模型 IQ`；子项也可以独立设置默认展开。所有选择都会在重新打开菜单和重启应用后保留。`恢复默认布局` 只重置这些布局偏好，不会改变语言、字号、提醒或自动更新设置。
...
`Codex 额度` 中的 `额度历史` 默认折叠，也可以在 `布局` 中设为默认展开。它提供 24 小时、7 天和 30 天三个范围；将鼠标移到曲线上，或在曲线上拖动，即可查看最接近时间的剩余量和相对上次记录的变化。
