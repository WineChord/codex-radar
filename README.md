...
| 额度历史 | 在本机保留 24 小时、7 天和 30 天周额度余量曲线，可逐点查看变化、重置与数据断档。 |
...
点击底部 `布局` 可统一调整顶层模块：拖动手柄或使用上移、下移按钮改变顺序，通过 `显示` 决定它是否出现在菜单中，并为显示的模块选择是否默认展开。模块内还有可折叠子项时，会在所属模块下方缩进显示，例如 `Codex 额度` 下的 `额度历史` 和 `Codex IQ` 下的 `全部模型 IQ`；子项也可以独立显示、隐藏或设置默认展开。所有选择都会在重新打开菜单和重启应用后保留。`恢复默认布局` 会恢复全部模块和子项，并重置它们的顺序与展开状态，不会改变语言、字号、提醒或自动更新设置。

首次显示时，底部工具栏上方会用一条轻提示说明布局能力。点击提示会直接进入布局编辑器；直接点击 `布局` 或关闭提示后，它都不会重复出现。

隐藏只改变菜单展示，不会停止额度历史记录、提醒或重置卡自动使用。当前结论、紧急提示和连接错误不参与排序，也不能隐藏；重置卡或更新遇到需要处理的状态时，对应模块会临时显示并锁定展开，问题解除后再恢复用户原来的显示与展开偏好。

### 额度历史

`Codex 额度` 中的 `额度历史` 默认显示但保持折叠；可以在 `布局` 中设为默认展开，也可以完全隐藏。它提供 24 小时、7 天和 30 天三个范围；将鼠标移到曲线上，或在曲线上拖动，即可查看最接近时间的剩余量和相对上次记录的变化。隐藏曲线不会停止本机采样。

应用只在成功读到本机周额度后记录真实采样点。连续运行时至少间隔 5 分钟保留一次记录；余额发生明显变化或重置时间变化时会立即保留。曲线不会跨过长时间断档补线，可靠的向上跳变只标记为“观察到重置”，不会推断是周期重置、重置卡还是其他服务端修正。

历史从启用该版本后开始积累，不会伪造或倒推此前数据；本机最多保留 31 天。区间底部的“观测消耗”只汇总实际观测到的余额下降，不能替代服务端用量账单。
...
- macOS 本机额度优先通过当前用户专属、已经登录的 Codex 受管会话读取；不可用时回退到独立的本机 app-server。Windows 通过当前用户的本机 Codex app-server 读取额度。两条链路都不会读取、复制或缓存登录凭证，也不会把额度上传到 CodexRadar。
